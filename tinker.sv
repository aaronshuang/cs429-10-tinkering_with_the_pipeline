...
module tinker_core (
    ...
    // ==========================================
    // GLOBAL FLUSH SIGNAL (Mispredict Recovery)
    // ==========================================
    wire bu_valid, bu_mispredicted; 
    wire [63:0] bu_correct_tgt; 
    ...
    wire pred_taken; 
    wire [63:0] pred_tgt;
    wire bu_actual_taken; 
    wire [63:0] bu_branch_pc;

    branch_predictor #(64) bp_unit (
        .clk(clk), .reset(reset),
        .fetch_pc(pc), .predict_taken(pred_taken), .predict_target(pred_tgt),
        .update_en(bu_valid), .update_pc(bu_branch_pc), .actual_taken(bu_actual_taken), .actual_target(bu_correct_tgt)
    );
    ...
    wire [5:0] cdb1_tag, cdb2_tag; 
    wire [4:0] cdb1_rd, cdb2_rd; 
    wire [63:0] cdb1_data, cdb2_data;
    
    // ==========================================
    // REORDER BUFFER (ROB)
    // ==========================================
    wire [5:0] rob_tag_A, rob_tag_B; wire rob_full; reg alloc_A, alloc_B;
    ...
    reorder_buffer #(64) rob_unit (
        ...
        .tag_A(rob_tag_A), .tag_B(rob_tag_B), .full(rob_full), 
    ...
    // ==========================================
    // REGISTER FILE (Written by ROB Commits)
    // ==========================================
    wire [63:0] rs_val_A, rt_val_A, rd_val_A, rs_val_B, rt_val_B, rd_val_B, r31_val;
    register_file reg_file (
        .clk(clk), .reset(reset), 
        .write_enable_1(com_A_vld), .data_1(com_A_dat), .rd_w1(com_A_rd), 
        .write_enable_2(com_B_vld), .data_2(com_B_dat), .rd_w2(com_B_rd), 
    ...
    rat_superscalar rat_unit (
        .clk(clk), .reset(reset), .flush(sys_flush), 
        .rs_A(rs_A), .rt_A(rt_A), .rd_A(rd_A), .val_rs_A(rs_val_A_rat), .val_rt_A(rt_val_A_rat), .val_rd_A(rd_val_A_rat), .tag_rs_A(rs_tag_A), .tag_rt_A(rt_tag_A), .tag_rd_A(rd_tag_A), .alloc_A(alloc_A), .alloc_tag_A(rob_tag_A), 
        .rs_B(rs_B), .rt_B(rt_B), .rd_B(rd_B), .val_rs_B(rs_val_B_rat), .val_rt_B(rt_val_B_rat), .val_rd_B(rd_val_B_rat), .tag_rs_B(rs_tag_B), .tag_rt_B(rt_tag_B), .tag_rd_B(rd_tag_B), .alloc_B(alloc_B), .alloc_tag_B(rob_tag_B), 
        .commit_A_valid(com_A_vld), .commit_A_tag(com_A_tag), .commit_A_rd(com_A_rd), 
        .commit_B_valid(com_B_vld), .commit_B_tag(com_B_tag), .commit_B_rd(com_B_rd)
    );
    ...
    reservation_station #(6'd1) rs_alu_1 (.clk(clk), .reset(reset), .flush(sys_flush), .alloc_en(alu1_alloc), .op_in(rs1_op), .rd_in(rs1_rd), .pc_in(64'b0), .pred_tgt_in(64'b0), .val_vj(rs1_vj_v), .vj_in(rs1_vj_d), .qj_in(rs1_qj), .val_vk(rs1_vk_v), .vk_in(rs1_vk_d), .qk_in(rs1_qk), .cdb1_valid(cdb1_valid), .cdb1_tag(cdb1_tag), .cdb1_data(cdb1_data), .cdb2_valid(cdb2_valid), .cdb2_tag(cdb2_tag), .cdb2_data(cdb2_data), .ready_to_fire(alu1_fire), .op_out(alu1_op), .rd_out(alu1_rd), .pc_out(), .pred_tgt_out(), .vj_out(alu1_vj), .vk_out(alu1_vk), .tag_out(alu1_t), .fire_ack(alu1_fire && alu1_ready), .busy(rs_alu1_busy));
    reservation_station #(6'd2) rs_alu_2 (.clk(clk), .reset(reset), .flush(sys_flush), .alloc_en(alu2_alloc), .op_in(rs2_op), .rd_in(rs2_rd), .pc_in(64'b0), .pred_tgt_in(64'b0), .val_vj(rs2_vj_v), .vj_in(rs2_vj_d), .qj_in(rs2_qj), .val_vk(rs2_vk_v), .vk_in(rs2_vk_d), .qk_in(rs2_qk), .cdb1_valid(cdb1_valid), .cdb1_tag(cdb1_tag), .cdb1_data(cdb1_data), .cdb2_valid(cdb2_valid), .cdb2_tag(cdb2_tag), .cdb2_data(cdb2_data), .ready_to_fire(alu2_fire), .op_out(alu2_op), .rd_out(alu2_rd), .pc_out(), .pred_tgt_out(), .vj_out(alu2_vj), .vk_out(alu2_vk), .tag_out(alu2_t), .fire_ack(alu2_fire && alu2_ready), .busy(rs_alu2_busy));
    reservation_station #(6'd3) rs_fpu_1 (.clk(clk), .reset(reset), .flush(sys_flush), .alloc_en(fpu1_alloc), .op_in(rs3_op), .rd_in(rs3_rd), .pc_in(64'b0), .pred_tgt_in(64'b0), .val_vj(rs3_vj_v), .vj_in(rs3_vj_d), .qj_in(rs3_qj), .val_vk(rs3_vk_v), .vk_in(rs3_vk_d), .qk_in(rs3_qk), .cdb1_valid(cdb1_valid), .cdb1_tag(cdb1_tag), .cdb1_data(cdb1_data), .cdb2_valid(cdb2_valid), .cdb2_tag(cdb2_tag), .cdb2_data(cdb2_data), .ready_to_fire(fpu1_fire), .op_out(fpu1_op), .rd_out(fpu1_rd), .pc_out(), .pred_tgt_out(), .vj_out(fpu1_vj), .vk_out(fpu1_vk), .tag_out(fpu1_t), .fire_ack(fpu1_fire && fpu_ready), .busy(rs_fpu1_busy));
    reservation_station #(6'd4) rs_branch(.clk(clk), .reset(reset), .flush(sys_flush), .alloc_en(bu_alloc),   .op_in(rs4_op), .rd_in(rs4_rd), .pc_in(rs4_pc), .pred_tgt_in(rs4_pred), .val_vj(rs4_vj_v), .vj_in(rs4_vj_d), .qj_in(rs4_qj), .val_vk(rs4_vk_v), .vk_in(rs4_vk_d), .qk_in(rs4_qk), .cdb1_valid(cdb1_valid), .cdb1_tag(cdb1_tag), .cdb1_data(cdb1_data), .cdb2_valid(cdb2_valid), .cdb2_tag(cdb2_tag), .cdb2_data(cdb2_data), .ready_to_fire(bu_fire),   .op_out(bu_op),   .rd_out(bu_rd),   .pc_out(bu_pc_out), .pred_tgt_out(bu_pred_out), .vj_out(bu_vj),   .vk_out(bu_vk),   .tag_out(bu_t),   .fire_ack(bu_fire && bu_ready),   .busy(rs_bu_busy));

    // ==========================================
    // LSQ & MEMORY SUBSYSTEM
    // ==========================================
                    ...
                    if (op_A == 5'h0f) begin hlt <= 1'b1; end 
                        ...
                        if (br_A) begin
                            if (!rs_bu_busy) begin 
                                ...
                                rs4_pc<=pc; rs4_pred<=pred_tgt;
                            end
                        end else if (is_mem_A) begin
                            if (!lsq_full) begin
                                lsq_alloc <= 1; alloc_A <= (op_A == 5'h10); issued_A = 1;
