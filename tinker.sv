...
module tinker_core (
    ...
    localparam FETCH = 2'd0, ISSUE = 2'd1, WAIT_DRAIN = 2'd2, EXEC_MEM = 2'd3;
    ...
    wire pred_taken; wire [63:0] pred_tgt; wire bu_actual_taken; wire [63:0] bu_branch_pc;
    ...
    reorder_buffer #(64) rob_unit (.clk(clk), .reset(reset), .flush(sys_flush), .alloc_A(alloc_A), .rd_A(rd_A), .alloc_B(alloc_B), .rd_B(rd_B), .tag_A(rob_tag_A), .tag_B(rob_tag_B), .full(rob_full), .empty(rob_empty), .cdb1_valid(cdb1_valid), .cdb1_tag(cdb1_tag), .cdb1_data(cdb1_data), .cdb2_valid(cdb2_valid), .cdb2_tag(cdb2_tag), .cdb2_data(cdb2_data), .commit_A_valid(com_A_vld), .commit_A_rd(com_A_rd), .commit_A_data(com_A_dat), .commit_A_tag(com_A_tag), .commit_B_valid(com_B_vld), .commit_B_rd(com_B_rd), .commit_B_data(com_B_dat), .commit_B_tag(com_B_tag));
    ...
    // --- FIX 1: ALU Operand Override Logic ---
    wire use_rd_A = (op_A == 5'h19 || op_A == 5'h1b || op_A == 5'h05 || op_A == 5'h07 || op_A == 5'h12);
    ...
    wire use_rd_B = (op_B == 5'h19 || op_B == 5'h1b || op_B == 5'h05 || op_B == 5'h07 || op_B == 5'h12);
    ...
    reg [63:0] rs4_pc, rs4_pred; reg [5:0] rs1_tag, rs2_tag, rs3_tag, rs4_tag, lsq_alloc_tag; 
    reg alu1_alloc, alu2_alloc, fpu1_alloc, lsq_alloc, bu_alloc;

    reg lsq_is_store; reg [4:0] lsq_rd; reg [63:0] lsq_imm; reg lsq_av_vld; reg [63:0] lsq_av_dat; reg [5:0] lsq_av_tag; reg lsq_dv_vld; reg [63:0] lsq_dv_dat; reg [5:0] lsq_dv_tag;
    ...
    wire [63:0] alu1_vj, alu1_vk, alu2_vj, alu2_vk, fpu1_vj, fpu1_vk, bu_vj, bu_vk, bu_vl, bu_pc_out, bu_pred_out;
    wire [5:0] alu1_t, alu2_t, fpu1_t, bu_t; wire alu1_ready, alu2_ready, fpu_ready, bu_ready;

    // Notice the empty 3rd operand on ALU/FPU: .val_vl(1'b1), .vl_in(64'b0), .ql_in(6'b0)
    ...
    wire [63:0] m_wdata = is_exec_mem ? (pc + 4) : lsq_mem_wdata; // PC+4 is Return Address
    ...
    // Notice the c_val mapped to bu_vl
    wire [5:0] bu_tout; wire [4:0] bu_rdout; wire [63:0] bu_res; wire bu_ack;
    branch_unit branch_exec (.clk(clk), .reset(reset), .flush(sys_flush), .valid_in(bu_fire), .op(bu_op), .pc(bu_pc_out), .predicted_target(bu_pred_out), .a_val(bu_vj), .b_val(bu_vk), .c_val(bu_vl), .imm(bu_vk), .tag_in(bu_t), .rd_in(bu_rd), .ready_in(bu_ready), .valid_out(bu_valid), .mispredicted(bu_mispredicted), .correct_target(bu_correct_tgt), .actual_taken(bu_actual_taken), .branch_pc(bu_branch_pc), .res_out(bu_res), .tag_out(bu_tout), .rd_out(bu_rdout), .ack_out(bu_ack));

    // 5-Way Arbiter
    wire sel1_lsq = lsq_vout; wire sel1_bu  = !sel1_lsq & bu_valid; wire sel1_a1  = !sel1_lsq & !sel1_bu & alu1_vout; wire sel1_a2  = !sel1_lsq & !sel1_bu & !sel1_a1 & alu2_vout; wire sel1_fpu = !sel1_lsq & !sel1_bu & !sel1_a1 & !sel1_a2 & fpu_vout;
    ...
    wire in2_bu  = bu_valid & ~sel1_bu; wire in2_a1  = alu1_vout & ~sel1_a1; wire in2_a2  = alu2_vout & ~sel1_a2; wire in2_fpu = fpu_vout & ~sel1_fpu;
    wire sel2_bu  = in2_bu; wire sel2_a1  = !sel2_bu & in2_a1; wire sel2_a2  = !sel2_bu & !sel2_a1 & in2_a2; wire sel2_fpu = !sel2_bu & !sel2_a1 & !sel2_a2 & in2_fpu;
    ...
    assign lsq_ack  = sel1_lsq; assign bu_ack   = sel1_bu | sel2_bu; assign alu1_ack = sel1_a1 | sel2_a1; assign alu2_ack = sel1_a2 | sel2_a2; assign fpu_ack  = sel1_fpu | sel2_fpu;
    ...
    always @(posedge clk) begin
        ...
        if (reset) begin 
            state <= FETCH; hlt <= 0; pc <= 64'h2000; alloc_A <= 0; alloc_B <= 0; alu1_alloc <= 0; alu2_alloc <= 0; fpu1_alloc <= 0; lsq_alloc <= 0; bu_alloc <= 0; 
        end 
        else if (sys_flush) begin
            state <= FETCH; pc <= bu_correct_tgt; 
                    ...
                    if (op_A == 5'h0f) begin 
                        ...
                        if (br_A) begin
                            if (!rs_bu_busy) begin 
                                ...
                                rs4_op<=op_A; rs4_rd<=rd_A; rs4_qj<=rs_tag_A; rs4_qk<=rt_tag_A;
                                rs4_vj_v<=rs_val_A_rat; rs4_vj_d<=rs_val_A; rs4_vk_v<=vk_vld_A; rs4_vk_d<=vk_dat_A; 
                                rs4_vl_v<=rd_val_A_rat; rs4_vl_d<=rd_val_A; rs4_ql<=rd_tag_A; // 3rd Operand Routed!
                                rs4_pc<=pc; rs4_pred<=pred_tgt; rs4_tag<=rob_tag_A;
                            end
                        end else if (is_mem_A) begin
                            if (!lsq_full) begin
                                lsq_alloc <= 1; alloc_A <= 1; issued_A = 1;
                            ...
                            if (op_B == 5'h0f) begin
                                ...
                                if (br_B) begin
                                    if (!rs_bu_busy && !bu_alloc) begin 
                                        ...
                                        rs4_op<=op_B; rs4_rd<=rd_B; rs4_qj<=rs_tag_B; rs4_qk<=rt_tag_B;
                                        rs4_vj_v<=rs_val_B_rat; rs4_vj_d<=rs_val_B; rs4_vk_v<=vk_vld_B; rs4_vk_d<=vk_dat_B; 
                                        rs4_vl_v<=rd_val_B_rat; rs4_vl_d<=rd_val_B; rs4_ql<=rd_tag_B; // 3rd Operand Routed!
                                    ...
                                    if (!rs_fpu1_busy && !fpu1_alloc) begin 
                                        ...
                                        rs3_op<=op_B; rs3_rd<=rd_B; rs3_vj_v<=vj_vld_B; rs3_vj_d<=vj_dat_B; rs3_qj<=qj_B; rs3_vk_v<=vk_vld_B; rs3_vk_d<=vk_dat_B; rs3_qk<=rt_tag_B;
                                        rs3_tag<=rob_tag_B;
                                    end
                                end
                            end
