...
module tinker_core (
    ...
    wire bp_update_en, bp_actual_taken; wire [63:0] bp_update_pc, bp_actual_target;
    ...
    wire [5:0] alu1_t, alu2_t, fpu1_t, bu_t_out; wire alu1_ready, alu2_ready, fpu_ready, bu_ready;
    ...
    FPU fpu        (.clk(clk), .reset(reset), .flush(sys_flush), .valid_in(fpu1_fire), .op(fpu1_op), .a(fpu1_vj), .b(fpu1_vk), .tag_in(fpu1_t), .rd_in(fpu1_rd), .ready_in(fpu_ready), .valid_out(fpu_vout), .res_out(fpu_res), .tag_out(fpu_tout), .rd_out(fpu_rdout), .ack_out(fpu_ack)); // INSTANCE NAME FIXED
    
    wire [5:0] bu_tout_w; wire [4:0] bu_rdout_w; wire [63:0] bu_res_w; wire bu_ack;
    ...
    assign cdb1_data  = sel1_lsq ? lsq_res   : (sel1_bu ? bu_res_w   : (sel1_a1 ? alu1_res   : (sel1_a2 ? alu2_res   : fpu_res)));
    ...
    assign cdb2_data  = sel2_bu ? bu_res_w   : (sel2_a1 ? alu1_res   : (sel2_a2 ? alu2_res   : fpu_res));
    ...
    always @(posedge clk) begin
        ...
        if (reset) begin 
            ...
            case (state)
                ...
                ISSUE: begin
                    if (op_A == 5'h0f) begin 
                        // Wait for ROB and Backend to drain completely before halting
                        ...
                        if (br_A) begin
                            if (!rs_bu_busy) begin 
                                ...
                                rs4_vl_v<=rd_val_A_rat; rs4_vl_d<=rd_val_A; rs4_ql<=rd_tag_A; 
                            ...
                            if (!lsq_full) begin
                                ...
                                lsq_av_vld <= vj_vld_A; lsq_av_dat <= vj_dat_A; lsq_av_tag <= qj_A;
                            ...
                            if (op_B == 5'h0f) begin
                                ...
                                if (br_B) begin
                                    if (!rs_bu_busy && !bu_alloc) begin 
                                        ...
                                        rs4_vl_v<=rd_val_B_rat; rs4_vl_d<=rd_val_B; rs4_ql<=rd_tag_B; 
                ...
                EXEC_MEM: begin
                    if (op_A == 5'h0d) state <= EXEC_MEM_WAIT; // Latency cycle for mem read
                    else begin
                ...
                EXEC_MEM_WAIT: begin
                    pc <= m_rdata; // Fetch Return Address
                    state <= FETCH;
                end
            endcase
        end
