...
module tb_comprehensive;
    ...
    initial begin
        ...
        assemble(32'h201C, 5'h19, 5'd7, 5'd0, 5'd0, 12'h02C); // ADDI R7, R0, 0x202C (Load Branch Target Address)
        assemble(32'h2020, 5'h0b, 5'd7, 5'd6, 5'd0, 12'd0);   // BNZ R6, target=R7  (Branch if R6!=0 to 0x202C)

        // 6. THE POISON INSTRUCTION (Should be fetched speculatively, but FLUSHED)
        assemble(32'h2024, 5'h19, 5'd8, 5'd0, 5'd0, 12'hBAD); // ADDI R8, R0, 0xBAD (Should NEVER commit to RegFile)
        assemble(32'h2028, 5'h0f, 5'd0, 5'd0, 5'd0, 12'd0);   // HALT (In case flush fails)

        // 7. CORRECT BRANCH TARGET
        assemble(32'h202C, 5'h19, 5'd9, 5'd0, 5'd0, 12'd42);  // ADDI R9, R0, 42 (Proof that branch succeeded)
        assemble(32'h2030, 5'h0f, 5'd0, 5'd0, 5'd0, 12'd0);   // HALT
