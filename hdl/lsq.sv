module lsq #(parameter DEPTH = 4) (
    ...
    input data_vld, input [63:0] data_val, input [5:0] data_tag,
    input [5:0] alloc_tag,

    output full,
    output [1:0] current_tail, // Used for dynamic tag generation
    ...
    output wire mem_read, output wire mem_write,
    output wire [63:0] mem_addr, output wire [63:0] mem_wdata,
    input [63:0] mem_rdata,

    // CDB Broadcast (For Loads)
    output reg cdb_valid, output reg [5:0] cdb_tag, output reg [4:0] cdb_rd, output reg [63:0] cdb_data,
    input cdb_ack
);
    // Queue State
    ...
    reg [1:0] head, tail; reg [2:0] count;
    ...
    always @(posedge clk) begin
        if (reset) begin 
            ...
            if (push) begin
                ...
                d_vld[tail] <= data_vld; d_val[tail] <= data_val; d_tag[tail] <= data_tag;
                tag[tail] <= alloc_tag;
            end

            // Dual CDB Snooping for all valid entries
            ...
            // Execution / Writeback (Load)
            if (head_ready && !head_is_st && !waiting_cdb) begin
                cdb_valid <= 1;
                cdb_data <= mem_rdata; // Combinatorial read from memory
                cdb_tag <= tag[head];
                cdb_rd <= rd[head];
                waiting_cdb <= 1;
                valid[head] <= 0;
            end else if (pop_store) begin
                valid[head] <= 0; // Store finishes instantly
            end
        end
    end
endmodule