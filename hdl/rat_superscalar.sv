module rat_superscalar (
    ...
    // --- Instruction A Ports ---
    input [4:0] rs_A, rt_A, rd_A, output val_rs_A, val_rt_A, val_rd_A,
    output [5:0] tag_rs_A, tag_rt_A, tag_rd_A, input alloc_A, input [5:0] alloc_tag_A,

    // --- Instruction B Ports ---
    input [4:0] rs_B, rt_B, rd_B, output val_rs_B, val_rt_B, val_rd_B,
    output [5:0] tag_rs_B, tag_rt_B, tag_rd_B, input alloc_B, input [5:0] alloc_tag_B,

    // --- ROB Commit Snooping (Replaced CDB Snooping) ---
    ...
    assign val_rd_A = (rd_A == 0) ? 1'b1 : valid[rd_A];
    ...
    wire B_reads_A_rd = (alloc_A && rd_A == rd_B && rd_B != 0);
    ...
    assign val_rd_B = B_reads_A_rd ? 1'b0 : (rd_B == 0 ? 1'b1 : valid[rd_B]);
    assign tag_rd_B = B_reads_A_rd ? alloc_tag_A : tags[rd_B]; 

    always @(posedge clk) begin
        if (reset) begin
            for (i=0; i<32; i=i+1) begin valid[i] <= 1'b1; tags[i] <= 6'b0; end
        end else begin
            // Snooping the ROB Commits instead of the CDB
            if (commit_A_valid && commit_A_rd != 0) begin
                if (!valid[commit_A_rd] && tags[commit_A_rd] == commit_A_tag) valid[commit_A_rd] <= 1'b1;
            end
            if (commit_B_valid && commit_B_rd != 0) begin
                if (!valid[commit_B_rd] && tags[commit_B_rd] == commit_B_tag) valid[commit_B_rd] <= 1'b1;
            end
